...
async function MonthStatsPDF() {
  ...
  // const ordersFetch = useGetOrders();

  // console.log('ordersFetch', ordersFetch);

  const loadStats = async () => {
    // const totalValue = orders?.reduce((acc, obj) => acc + obj.total, 0);

    // if (ordersFetch.orders) {
    //   // setStats({
    //   //   totalValue,
    //   // });
    //   setRenderPDF(true);
  ...
  //   //             Total de vendas: {ordersFetch.orders?.length ? ordersFetch.orders.length : 0}
  //   //           </Text>
  //   //           <Text style={[styles.pageTitle, { fontSize: '12px', textAlign: 'center', flex: 1 }]}>
  //   //             Total de produtos vendidos: {ordersFetch.orders?.reduce((acc, obj) => acc + obj.qt_products, 0)}
  //   //           </Text>
  //   //         </View>
